...
//javascript:(function(Q,R,I,L,B){let C=B.getContext("2d"),z=parseFloat(getComputedStyle(B).getPropertyValue(R))/9,W=z/2,M=(x,y,b,n)=>["mousedown","mouseup"].map(e=>n.dispatchEvent(new MouseEvent(e,{clientX:x,clientY:y,button:b}))),p=(a,b,n)=>{let e=document.querySelectorAll(".numpad-item")[n-1],{top:x,left:y}=B[Q](),{top:t,left:l}=e[Q]();M(y+a*z+W,x+b*z+W,0,B);M(l,t,0,e)},m=(a,b)=>{let[c,d]=[b%9,b/9|0],e=3*(d/3|0),f=3*(c/3|0),g=0,h=e,i=0,j;for(;h<e+3;h++)for(j=f;j<f+3;j++,i++)g|=a[h*9+j]|a[d*9+i]|a[i*9+c];return g^511},U=(a,b,c)=>{let[d,e]=[b%9,b/9|0],f=9*e,g=d,h=1,i=1,j=1,k=3*(e/3|0),l;for(;k<f+3;++k)for(l=3*(d/3|0);l+3;++l,++f,g+=9)if(h&&f-d&&a[f]&c&&(h=0)||i&&g-d&&a[g]&c&&(i=0)||j&&k*9+l-d&&a[k*9+l]&c&&(j=0)||h+i+j)return!1;return h+i+j},Z=a=>{let b=a.map((e,i)=>e?0:m(a,i)),c=-1,d=1e2,e=0,f,g,h;for(;e<81;e++)if(!a[e]){for(g=1,h=b[e],f=0;h;g<<=1)if(h&g&&++f)if(U(b,e,g)&&(f=1)&&(b[e]=g))break;else h^=g;if(f<d&&(c=e)&&!(d=f))break}return[c,b[c]]},G=_=>{let s=B[R]/9-2,z=74,g=(v,[n,m])=>n<=v&&v<=m,f=[[910,1007],[1239,1287],[1421,1481],[1272,1396],[1659,1709],[1769,1896],[1236,1293],[1895,1963],[1949,2045]],l=[[639,740],[916,1007],[1159,1231],[1e3,1170],[1081,1151],[1100,1159],[586,672],[1147,1231],[1233,1294]],c=(a,r=[],i=0)=>{for(let l=a[L];i<l;i+=4)r.push(a.slice(i,i+4));return r},T=(X,Y)=>{let x=B[R]/2+X*s-37,y=B.height/2+Y*s-37,t=([r,g,b])=>((r<<16)|(g<<8)|b)==3426401,q=c(C[I](x,y-z*.3,z,z).data).filter(t)[L],c2=c(C[I](x+z*.34,y+z*.2,z,z).data).filter(t)[L],i=0;for(;i<9;i++)if(g(q,f[i])&&g(c2,l[i]))return++i;return 0},n=[],i=0;for(;i<81;i++)n.push(2**T(i%9-4,(i/9|0)-4)/2|0);return n},S=b=>{let c=_=>{let[d,e]=Z(b),f=1;if(d<0)return!0;for(;e;f<<=1)if(e&f&&(b[d]=f))if(c())return!0;else e^=f;b[d]=0};if(c())return b.map(n=>{for(var b=0;n;n>>=1,b++);return b})},r=G(),s=S(r),i=0;for(;i<81;i++)p(i%9,i/9|0,s[i])})("getBoundingClientRect","width","getImageData","length",document.querySelector("#game>canvas"));
